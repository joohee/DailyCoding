...
class SNSObject:
    ...
    def push_apns(self, section, client, pushToken, message=None):
        ...
        if config.sections() != None:
            try:
                key_filename = config.get(section, 'apns_key_filename')
                cer_filename = config.get(section, 'apns_cer_filename')
            except:
    ...
    def push_gcm(self, section, client, registrationId, message=None):
        ...
        if config.sections() != None:
            try:
                server_key = config.get(section, 'serverKey')
